...
if project == "Day 1: Band Name Generator":
    ...
    if st.button("Calculate Split"):
        if total_bill > 0:
            # Simple math logic converted from standard Python math
            tip_amount = total_bill * (tip_percentage / 100)
            grand_total = total_bill + tip_amount
            per_person_pay = grand_total / split_people
            
            # Displays the final amount rounded nicely
    ...
    # Using a code block to preserve your exact ASCII art layout
    st.code('''
 _                                     _     _                 _ 
| |                                   (_)   | |               | |
...
*******************************************************************************
    ...
    # Streamlit uses "session state" to remember choices as the user advances
    if "step" not in st.session_state:
        st.session_state.step = 1

    # Restart button so she can play again easily
